...
var o = (l,e,s)=>(S(l, typeof e != "symbol" ? e + "" : e, s),
s);
const P = {
    by: "by",
    across: "Across",
  ...
  , B = {
    ...
    id: {
        ...
        letters: "huruf",
        congrats1: "Keren!",
        congrats2: "Kamu berhasil menyelesaikan Teka-Teki Silang ini",
        checkBefore: "",
        checkOf: "dari",
        checkAfter: "jawabanmu benar.",
        terms: "Syarat Penggunaan",
        makeYourOwn: "Buat TTS-mu sendiri menggunakan aplikasi gratis EclipseCrossword."
    },
    it: {
        by: "di",
        across: "Orizzontali",
    ...
    jv: {
        ...
        across: "Nengen",
        down: "Mudhun",
        checkAnswers: "Cek wangsulanku",
        solve: "nonton kunci",
        close: "Kukut",
        letters: "huruf",
        congrats1: "Wangun!",
        congrats2: "Sampeyan wis sukses ngrampungake TTS iki",
        checkBefore: "",
        checkOf: "saka",
        checkAfter: "wangsulanku bener.",
        terms: "Katentuan Pangginaan",
        makeYourOwn: "Gawea TTS dhewe nggunakake EclipseCrossword tanpa bayar. "
    },
    pt: {
        by: "de",
        across: "Horizontais",
...
class D {
    constructor(e) {
        ...
        this._isInCtor = !0,
        window.visualViewport ? window.visualViewport.addEventListener("resize", this._onResize.bind(this)) : window.addEventListener("resize", this._onResize.bind(this)),
    ...
    _onResize() {
        ...
        window.visualViewport ? this._viewportHeight = window.visualViewport.height : this._viewportHeight = Math.min(window.innerHeight, document.documentElement.clientHeight),
        this._isInCtor || this._changeCallback()
    }
    get isPhoneSize() {
...
class O {
    constructor(e) {
        o(this, "_words");
        o(this, "_grid");
        if (!e || e.version > 1 || !e.words || e.words.length === 0)
            throw new Error;
        ...
        for (let r = 0; r < this._words.length; r++) {
            ...
            for (let a = 0; a < n.length; a++) {
                ...
                h ? (a === 0 && n.number && (h.number = n.number),
                n.direction === 0 ? h.across = n : h.down = n) : (h = {
                    number: a ? 0 : n.number
                },
                n.direction === 0 ? h.across = n : h.down = n,
                this._grid.set(u, m, h))
            }
        }
...
class V {
    constructor(e) {
        ...
        this._remainingChecks = e.checks =Infinity,
    ...
    takeMobilePane() {
        ...
        this._curtain.addEventListener("click", this.onCloseMobilePaneClick.bind(this)),
        this._mobilePane = document.getElementById("mobile-pane"),
        this._mobilePane.addEventListener("click", this.openMobilePane.bind(this)),
        this._mobilePaneBackButton = document.getElementById("mobile-pane-back"),
        this._mobilePaneBackButton.addEventListener("click", this.onCloseMobilePaneClick.bind(this)),
    ...
    selectWordAt(e, s) {
        ...
        if (!!t)
            if (this._selectedWord && t.across === this._selectedWord) {
                ...
                t.across && t.down ? !this._lastSelectedWord || this._lastSelectedWord.direction === 0 ? this.selectWord(t.across) : this.selectWord(t.down) : t.across ? this.selectWord(t.across) : this.selectWord(t.down)
    }
    deselect() {
    ...
    selectCharacter(e) {
        if (this._isLocked || !this._selectedWord || this._selectedCharacter === e)
            return;
    ...
    typeCharacter(e) {
        ...
        i.isLocked || (i.contents = e !== null ? e : void 0,
        i.ui && (i.ui.rect.classList.remove("error"),
        i.ui.text.classList.remove("error"),
        i.ui.text.textContent = e)),
        this._isDirty = !0,
    ...
    setWordErrorState(e, s) {
        for (let t = 0; t < e.length; t++) {
            ...
            i.ui.text.classList.add("error")) : (i.ui.rect.classList.remove("error"),
            i.ui.text.classList.remove("error")))
        }
    }
    solveWord(e) {
        if (e && e.stopPropagation(),
        !this._selectedWord || !this._remainingSolves)
            return;
        const s = this._selectedWord;
        !s.word || this.enterWord(s.word, !0) === v.Success && (this._remainingSolves--,
        this.updateSolveCounter())
    }
    enterWord(e, s=!1) {
        if (this._isLocked)
        ...
        t.length !== e.length)
            return v.WrongLength;
        for (let i = 0; i < e.length; i++) {
            this._selectedCharacter = i,
            this.typeCharacter(e[i]);
            const c = this._crossword.cellFor(t, i);
            c.isLocked || s && (c.isLocked = !0,
            c.ui && (c.ui.rect.classList.add("solved"),
            c.ui.text.classList.add("solved")))
        }
    ...
    updateSolveCounter() {
        ...
        this._solveCounter.innerText = this._remainingSolves === 0 || this._remainingSolves === Infinity ? "" : `(${this._remainingSolves})`,
        this._mobilePaneSolveButton.style.display = !this._remainingSolves || this.areAllLettersInSelectedWordLocked() ? "none" : "",
        this._mobilePaneSolveCounter.innerText = this._remainingSolves === 0 || this._remainingSolves === Infinity ? "" : `(${this._remainingSolves})`
    }
    isWordCorrect(e) {
        if (e.word) {
            const s = e.word;
    ...
    checkPuzzle(e) {
        ...
        t ? (this._remainingChecks = 0,
        this._checkMessage.classList.remove("open"),
        this._congratsMessage.classList.add("open")) : (this._checkCorrectWords.innerText = s.toString(),
        this._checkWordCount.innerText = this._crossword.words.length.toString(),
        this._checkMessage.classList.add("open"),
        this._congratsMessage.classList.remove("open")),
        (t || !this._remainingChecks) && (this._isLocked = !0,
        this._isDirty = !1,
    ...
    updateCheckCounter() {
        this._remainingChecks || (this._checkButton.style.display = "none",
        this._mobilePaneCheckButton.style.display = "none"),
        this._checkCounter.innerText = this._remainingChecks === 0 || this._remainingChecks === Infinity ? "" : `(${this._remainingChecks})`,
        this._mobilePaneCheckCounter.innerText = this._remainingChecks === 0 || this._remainingChecks === Infinity ? "" : `(${this._remainingChecks})`
    }
    keyDown(e) {
        if (!(!e || e.defaultPrevented || e.keyCode === 229)) {
            ...
            case "ArrowDown":
                ...
                return;
            case "Enter":
                this._isPaneOpen && this._device.isTouchMode ? this._selectedWord && this.submitAnswer() : this.selectNextWord(),
                e.preventDefault();
                return;
            case "Up":
            case "ArrowUp":
                ...
                return;
            case "Tab":
                e.shiftKey ? this.selectPrevWord() : this.selectNextWord(),
                e.preventDefault();
                return;
            case "Home":
                this._device.isTouchMode || (this.selectCharacter(0),
                e.preventDefault());
                return;
            case "Left":
            case "ArrowLeft":
                this._device.isTouchMode || (this.selectPrevCharacter(),
                e.preventDefault());
                return;
            case "Backspace":
                this._device.isTouchMode || (this.typeCharacter(null),
                this.selectPrevCharacter(),
                e.preventDefault());
                break;
            case "Right":
            case "ArrowRight":
                this._device.isTouchMode || (this.selectNextCharacter(),
                e.preventDefault());
                return;
            case "Delete":
                this._device.isTouchMode || (this.typeCharacter(null),
            ...
            if (!this._isLocked && !this._device.isTouchMode && !e.isComposing && e.key && !e.ctrlKey && !e.altKey && !e.metaKey && e.key.length === 1) {
                this.typeCharacter(e.key.toUpperCase()),
    ...
    setMobilePaneOpenedCore(e) {
        e !== this._isPaneOpen && (e == null && (e = !this._isPaneOpen),
        this._isPaneOpen = e,
        e || (this._mobilePaneEntryBox.blur(),
        this.deselect()),
        e && this._selectedWord && window.requestAnimationFrame(()=>this._mobilePaneEntryBox.focus({
            preventScroll: !0
        })),
        this.updateMobilePane())
    }
    ...
    updateMobilePane() {
        this._isPaneOpen ? (this._mobilePane.classList.add("open"),
        this._selectedWord ? (this._mobilePane.classList.add("has-selection"),
        this._curtain.classList.remove("open"),
        this._mobilePaneClue.innerHTML = this._selectedWord.clueUI.innerHTML,
        this._mobilePaneLetterCount.innerText = this._selectedWord.length.toString(),
        this._mobilePaneEntryBox.placeholder = "\u25AA".repeat(this._selectedWord.length)) : (this._mobilePane.classList.remove("has-selection"),
        this._curtain.classList.add("open"))) : (this._mobilePane.classList.remove("open"),
        this._mobilePane.classList.remove("has-selection"),
        this._curtain.classList.remove("open")),
        !this._isLocked && this._device.isTouchMode && !this.areAllLettersInSelectedWordLocked() ? (this._mobilePaneEntryBox.style.removeProperty("display"),
        this._solveButton.style.removeProperty("display")) : (this._mobilePaneEntryBox.style.display = "none",
        this._solveButton.style.display = "none")
    }
    submitAnswer(e=!1) {
    ...
    areAllLettersInSelectedWordLocked() {
        ...
        if (!e)
            return !0;
        for (let s = 0; s < e.length; s++)
            if (!this._crossword.cellFor(e, s).isLocked)
                return !1;
        return !0
    }
    onBeforeUnload(e) {
    ...
    onSizeChanged() {
        this._device.isPhoneSize ? document.documentElement.classList.add("is-phone") : (document.documentElement.classList.remove("is-phone"),
        this.closeMobilePane()),
        this._device.isTouchMode ? document.documentElement.classList.add("is-touch") : document.documentElement.classList.remove("is-touch"),
        document.body.style.maxHeight = `${this._device.viewportHeight}px`,
        this.updateMobilePane()
    }
}
var v;
